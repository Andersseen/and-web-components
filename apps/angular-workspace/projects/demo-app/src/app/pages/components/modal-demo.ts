...
import { AndButton, AndModal } from '@andersseen/angular-components';
  ...
  template: `
    <div class="max-w-4xl mx-auto pb-12">
      ...
      <header class="mb-10 border-b border-border pb-10">
        <h1 class="text-3xl font-bold tracking-tight text-foreground m-0">Modal</h1>
        <p class="mt-4 text-lg text-muted-foreground max-w-2xl leading-relaxed">
          A dialog overlay that demands user attention. Use modals for confirmations, forms, or any content that
          requires a focused interaction.
        </p>
      </header>
      ...
      <section class="mb-12">
        ...
        <div class="rounded-xl border border-border bg-card overflow-hidden shadow-sm">
          <div class="p-10 flex flex-col items-center justify-center gap-6 min-h-[180px]">
            <p class="text-sm text-muted-foreground m-0">Trigger a modal dialog</p>
            <div class="flex gap-3">
              <and-button (click)="openDefault()">Open Modal</and-button>
              <and-button variant="destructive" (click)="openConfirm()"> Delete Item </and-button>
            </div>
          </div>
        </div>
      </section>
      ...
      <and-modal [open]="defaultOpen()" (myClose)="defaultOpen.set(false)">
        <h3 class="mt-0 text-lg font-semibold text-foreground">Modal Title</h3>
        <p class="text-sm text-muted-foreground leading-relaxed">
          This modal demonstrates the default behavior. It traps focus inside, closes on backdrop click, and supports
          the Escape key.
        </p>
        <div class="flex justify-end gap-2 mt-6">
          <and-button variant="ghost" (click)="defaultOpen.set(false)"> Cancel </and-button>
      ...
      <and-modal [open]="confirmOpen()" (myClose)="confirmOpen.set(false)">
        <h3 class="mt-0 text-lg font-semibold text-foreground">Are you sure?</h3>
        <p class="text-sm text-muted-foreground leading-relaxed">
          This action cannot be undone. The item will be permanently deleted from your account.
        </p>
        <div class="flex justify-end gap-2 mt-6">
          <and-button variant="ghost" (click)="confirmOpen.set(false)"> Cancel </and-button>
          <and-button variant="destructive" (click)="confirmOpen.set(false)"> Delete </and-button>
        </div>
      </and-modal>
      ...
      <section class="mb-12">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div class="rounded-xl border border-border bg-card p-5">
            <h3 class="text-sm font-semibold text-foreground mb-2">Confirmation</h3>
            <p class="text-sm text-muted-foreground m-0 leading-relaxed">
              Destructive actions, irreversible operations, account changes.
            </p>
          </div>
          <div class="rounded-xl border border-border bg-card p-5">
            <h3 class="text-sm font-semibold text-foreground mb-2">Forms</h3>
            <p class="text-sm text-muted-foreground m-0 leading-relaxed">
              Quick edits, new item creation, settings within context.
            </p>
          </div>
          <div class="rounded-xl border border-border bg-card p-5">
            <h3 class="text-sm font-semibold text-foreground mb-2">Information</h3>
            <p class="text-sm text-muted-foreground m-0 leading-relaxed">
