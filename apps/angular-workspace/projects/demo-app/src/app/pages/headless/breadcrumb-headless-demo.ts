    ...
    <div class="max-w-4xl mx-auto pb-12">
      ...
      <header class="mb-10 border-b border-border pb-10">
        ...
        <p class="mt-4 text-lg text-muted-foreground max-w-2xl leading-relaxed">
          A navigation component that shows the user's current location within a hierarchy. Manages ARIA semantics and
...
breadcrumb.actions.setCurrentItem('api');</code></pre>
      ...
      <section class="mb-12">
        ...
        <p class="mt-4 text-lg text-muted-foreground max-w-2xl leading-relaxed mb-6">
          The headless core handles ARIA attributes, current-page semantics, and keyboard navigation — you supply the
          markup.
        </p>
            ...
            <ol style="list-style: none; padding: 0; margin: 0; display: flex; gap: 6px;">
              @for (item of headlessItems(); track item.id; let last = $last) {
                <li style="display: inline-flex; align-items: center; gap: 6px;">
                  @if (!last) {
                    <a style="cursor: pointer;" (click)="headlessNavigate(item)">
                      {{ item.label }}
                    </a>
                    <span>/</span>
                  } @else {
                    <span
                      ><strong>{{ item.label }}</strong> (current)</span
                    >
                  }
                </li>
              }
...
export default class BreadcrumbHeadlessDemo {
  ...
  headlessAddItem() {
    ...
    this.headlessItems.set([...updated, { id, label: `Level ${current.length + 1}`, current: true }]);
  }
