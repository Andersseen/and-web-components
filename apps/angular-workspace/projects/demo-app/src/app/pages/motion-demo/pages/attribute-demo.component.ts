...
@Component({
  ...
  template: `
    <div class="max-w-4xl mx-auto flex flex-col gap-6" #demoRoot>
      ...
      <!-- ========== Enter Trigger ========== -->
      <section
        class="border border-border rounded-xl p-6 bg-card transition-shadow hover:shadow-lg duration-200"
      >
        <div class="mb-5">
          <span
            class="inline-block text-xs font-semibold uppercase tracking-wider text-primary bg-primary-50 px-2.5 py-1 rounded-full mb-2"
            >Enter</span
          >
        ...
        <div
          class="min-h-[120px] flex items-center justify-center border border-dashed border-border rounded-lg bg-muted mb-3 p-4"
        >
      ...
      <!-- API Note -->
      <div
        class="flex items-start gap-3 p-4 rounded-xl bg-muted border border-border"
      >
        <span class="text-xl flex-shrink-0">💡</span>
        <p class="text-sm text-muted-foreground leading-relaxed m-0">
          Initialize once with
          <code
            class="bg-card px-1.5 py-0.5 rounded border border-border text-xs"
            >initMotion()</code
          >.
        </p>
      </div>
