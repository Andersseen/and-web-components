...
describe('createTabs', () => {
  ...
  it('provides correct tab trigger props', () => {
    ...
    const selectedProps = tabs.getTriggerProps('tab-1');
    ...
    const unselectedProps = tabs.getTriggerProps('tab-2');
  ...
  it('provides correct tab content props', () => {
    ...
    const selectedProps = tabs.getContentProps('tab-1');
    expect(selectedProps.role).toBe('tabpanel');
    expect(selectedProps.tabindex).toBe(0);
    expect(selectedProps.hidden).toBe(false);
    expect(selectedProps['data-state']).toBe('active');

    const unselectedProps = tabs.getContentProps('tab-2');
  ...
  describe('keyboard navigation', () => {
    ...
    it('handles horizontal navigation (ArrowRight, ArrowLeft, Home, End)', () => {
      ...
      // ArrowRight -> tab-2
      tabs.handleTriggerKeyDown({ key: 'ArrowRight', preventDefault } as any, 'tab-1', tabIds);
      expect(preventDefault).toHaveBeenCalled();
      expect(tabs.state.selectedTab).toBe('tab-2');

      // ArrowLeft from tab-2 -> tab-1
      tabs.handleTriggerKeyDown({ key: 'ArrowLeft', preventDefault } as any, 'tab-2', tabIds);
      expect(tabs.state.selectedTab).toBe('tab-1');

      // ArrowLeft from tab-1 -> loops to tab-3
      tabs.handleTriggerKeyDown({ key: 'ArrowLeft', preventDefault } as any, 'tab-1', tabIds);
      expect(tabs.state.selectedTab).toBe('tab-3');

      // ArrowRight from tab-3 -> loops to tab-1
      tabs.handleTriggerKeyDown({ key: 'ArrowRight', preventDefault } as any, 'tab-3', tabIds);
      expect(tabs.state.selectedTab).toBe('tab-1');

      // End -> tab-3
      tabs.handleTriggerKeyDown({ key: 'End', preventDefault } as any, 'tab-1', tabIds);
      expect(tabs.state.selectedTab).toBe('tab-3');

      // Home -> tab-1
      tabs.handleTriggerKeyDown({ key: 'Home', preventDefault } as any, 'tab-3', tabIds);
      expect(tabs.state.selectedTab).toBe('tab-1');
    });
    ...
    it('handles vertical navigation (ArrowDown, ArrowUp)', () => {
      ...
      // ArrowDown -> tab-2
      tabs.handleTriggerKeyDown({ key: 'ArrowDown', preventDefault } as any, 'tab-1', tabIds);
      expect(preventDefault).toHaveBeenCalled();
      expect(tabs.state.selectedTab).toBe('tab-2');

      // ArrowUp from tab-2 -> tab-1
      tabs.handleTriggerKeyDown({ key: 'ArrowUp', preventDefault } as any, 'tab-2', tabIds);
      expect(tabs.state.selectedTab).toBe('tab-1');
    });
    ...
    it('ignores navigation for unknown tab', () => {
      ...
      tabs.handleTriggerKeyDown({ key: 'ArrowRight', preventDefault } as any, 'unknown-tab', tabIds);
      expect(preventDefault).not.toHaveBeenCalled();
    });
  });
});
