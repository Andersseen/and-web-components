...
| TD-9  | CI                 | ~~Pre-commit hook only runs Prettier (AGENTS.md claims ESLint too). CI does not run vanilla/motion tests.~~ Partially resolved: AGENTS.md now states the Prettier-only pre-commit correctly, and `ci-cd.yml` runs vanilla + motion tests. Still open (verified 2026-07-14): CI does **not** run `behaviors`, `icon-library`, `layout-core`, or the Angular/React/Vue wrapper smoke tests — three published adapter packages are never exercised by any pipeline. Fix: add the missing `pnpm -C packages/<pkg> test` steps to the `build-and-test` job in `.github/workflows/ci-cd.yml` (after `build:all`, so `dist/` folders exist).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Medium   |
