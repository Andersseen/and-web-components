...
| TD-28 | Release / versioning     | **Changesets majors the framework wrappers into an unpublishable `1.0.0` on every `web-components` release.** `@andersseen/web-components` is a `peerDependency` of `angular`/`react`/`vue-components`, and Changesets bumps peer-dependents as **major** by default (widening a peer range is breaking for consumers). On a `0.x` package that lands exactly on `1.0.0` — the one number those three packages can never publish, because it was unpublished from npm and npm permanently reserves unpublished versions. Hit twice so far (commit `7882f65`, then again 2026-07-23, both corrected by hand to mirror the `web-components` version). Options, none applied yet because each changes release policy: **(a)** set `___experimentalUnsafeOptions_WILL_CHANGE_IN_PATCH.onlyUpdatePeerDependentsWhenOutOfRange: true` — but on its own this is a trap, since the peer range is `workspace:*`, which is always "in range", so the wrappers would then _never_ auto-bump while still publishing an **exactly pinned** peer range, going stale and emitting peer warnings; it only works combined with a permissive published range such as `">=0.4.0 <1.0.0"`. **(b)** Move the wrappers past the burned number onto a real `1.x`/`2.x` line so majors stop colliding — rejected for now, the project is deliberately pre-1.0. **(c)** Keep correcting by hand after every `pnpm version-packages`, per the check added to AGENT-PLAYBOOKS P7. Currently on (c).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | High     |
