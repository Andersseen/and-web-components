...
| TD-12 | Forms              | ~~No native form participation. Zero references to `ElementInternals` / `formAssociated` / `attachInternals` anywhere in `packages/web-components` or `packages/vanilla-components` (verified by grep, 2026-07-14). Consequence: `and-input` and `and-select` inside a `<form>` do not appear in `FormData`, do not trigger implicit submission on Enter, and do not participate in native constraint validation.~~ Corrected + partially resolved (2026-07-14): that consequence was wrong for `and-input` — it uses `scoped: true` (light DOM), not Shadow DOM, so its inner `<input>` is a real descendant of the wrapping `<form>` and `FormData`/Enter-submit/native constraint validation already worked with **zero** `ElementInternals` code (verified live in a real browser). The only real gap was that a native `form.reset()` didn't resync the component's internal state with the browser-reset DOM value; fixed via a `reset` listener on `this.el.closest('form')` (see `and-input.tsx`, playbook **P9** step 0). **and-select is still open**: it renders a `<button>` + ARIA listbox with no real nested form control, so it genuinely needs `shadow: true` + `@AttachInternals()` per P9 steps 1-8 (tracked as R1.3 in ROADMAP.md). Every future form control (TD-13) must check P9 step 0 first to pick the right shape before implementing.                                                                                                                                                                                                                                                                                                                                                                                                   | High     |
| TD-13 | Component gaps     | **Missing basic form controls:** no checkbox, radio/radio-group, switch, textarea, or slider — in any layer (no headless module, no Stencil component, no vanilla component). These are the first components adopters look for. Each needs the full P1→P2 flow (headless module first, then Stencil), built form-associated from day one (TD-12/P9). Suggested order by effort/value: switch → checkbox → textarea → radio-group → slider.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | High     |
| TD-14 | Docs               | **No per-component user documentation site.** `apps/astro-landing` has a single page (`src/pages/index.astro`); Storybook shows stories but does not document props/events/slots/CSS-vars with copy-paste examples. The generated per-component `readme.md` files contain exactly this API data but are published nowhere. Adoption blocker. Candidate fix: Astro Starlight site (dogfoods `@andersseen/astro`) that renders the generated readmes plus hand-written usage guides.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Medium   |
