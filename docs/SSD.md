...
- **Per-component files:** `and-<name>.tsx` (logic), `and-<name>.css` (`:host`
  ...
  `and-<name>.spec.tsx` (Vitest spec), optionally `and-<name>.e2e.ts`
  (Puppeteer, deprecated) and `and-<name>.types.ts`. `readme.md` files are
  auto-generated by Stencil (`docs-readme` target) — never edit by hand.
...
| TD-3  | Tests              | Headless `carousel`, `input`, `menu` untested; 8 Stencil components lack specs; `and-code`/`and-skeleton` lack stories.                                                                                                                          | High     |
...
| TD-7  | Dependencies       | `packages/web-components` mixes Storybook 10 core with `@storybook/manager-api`/`@storybook/theming` v8; leftover `jest`, `jest-cli`, `@types/jest`, `puppeteer` devDeps alongside Vitest; root `wrangler` pinned to v3.                         | Medium   |
