...
| Add an **icon**                                                                     | `packages/icon-library/src/icons.ts` (SVG const) + `src/registry.ts` if registry list changes. `dist/icons.css` (the CSS-only `and-icon="name"` stylesheet) regenerates automatically from `ALL_ICONS` on `pnpm build:icons` — nothing to hand-edit. | `packages/icon-library/dist/icons.css` (generated)                     |
