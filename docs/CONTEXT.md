...
Open items as of 2026-07-15: **TD-11** (Angular wrapper tree-shaking — upstream
bug, don't try to fix locally), **TD-12** (`and-select` still needs
`ElementInternals` form participation — High; `and-input` resolved, see P9 step
0), **TD-13** (missing form controls: checkbox/radio/switch/textarea/slider —
High), **TD-15** (no browser e2e / visual regression). **TD-14 is factually
resolved** — the docs site exists (`apps/docs`, §4.1) — but SSD §15 / ROADMAP
R2.6 still list it open; formal closure (plus registering new TD-16, hand-
synced docs API tables) is PLAN F0 — do not re-create the docs site. TD-9 closed
2026-07-14 (CI test matrix complete). TD-7 has one optional leftover (root
`wrangler` v3 bump). Everything else (TD-1…TD-6, TD-8, TD-10) is resolved.
